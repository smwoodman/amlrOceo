...
#' net.station.header  <- tbl(con, "NET_STATION_HEADER") %>% collect()
#' amlr.station.header <- tbl(con, "AMLR_STATION_HEADER") %>% collect()
#' tucker.header.data  <- tbl(con, "TUCKER_HEADER_DATA") %>% collect()
#' 
#' calculate_filtered_volume(net, net.station.header, amlr.station.header, tucker.header.data)
#' 
#' @export
calculate_filtered_volume <- function(net, net.station.header, amlr.station.header, 
  ...
  net %>% 
    ...
    mutate(reading_na = final_reading == 0 | is.na(final_reading) | is.na(initial_reading), 
           ...
           volume = case_when(
             net_type == "I" ~ if_else(reading_na, 3850, (final_reading - initial_reading) * 0.0752),
             net_type == "B" ~ if_else(reading_na, 389, (final_reading - initial_reading) * 0.00758), 
             net_type %in% c("VRD", "VRS") ~ 
               if_else(reading_na, 133.5, (final_reading - initial_reading) * 0.00664), 
             net_type == "T" ~ if_else(amlr_cruise == "AMLR2011", tot_dist * 4, Swept_volume), 
             TRUE ~ NA_real_
           ), 
           depth_fished_isnull = if_else(net_type == "T", FALSE, replace_na(depth_fished, 0) == 0), 
           depth_fished = if_else( # must happen after depth_fished_isnull
             net_type == "T", Maximum_Depth, 
             if_else(replace_na(depth_fished, 0) == 0, as.integer(170), depth_fished))) %>% 
    select(net_id, net_station_header_id, amlr_cruise, amlr_station, amlr_station_header_id,
